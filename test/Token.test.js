...
contract("Token", ([deployer, receiver]) => {
  ...
  describe("deployment", () => {
    ...
    it("tracks the symbol", async () => {
      const result = await token.sumbol();
      result.should.equal(sumbol);
    });
    ...
    describe("sending tokens", () => {
      ...
      describe("failure", async () => {
        it("rejects insufficient balances", async () => {
          let invalidAmount;
          invalidAmount = tokens(100000000); // 100 million - grater thn total supply
          await token
            .transfer(receiver, invalidAmount, { from: deployer })
            .should.be.rejectedWith(EVM_REVERT);

          // Attempt transfers token, when you have none
          invalidAmount = tokens(10); // 100 million - grater thn total supply
          await token
